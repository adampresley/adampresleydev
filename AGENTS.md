...
3. Once you complete a task, always execute both `goimports` and `go build` to ensure the changes do not break anything. The application lives in the `cmd/adampresleydev` directory, so that is where `go build` needs to be run from.
